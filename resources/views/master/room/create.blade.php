...
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">Add Room</h2>
</div>

<!-- BEGIN: Form Layout -->
<form action="{{ Route('room.post.store') }}" method="post">
    @csrf
        ...
        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
            ...
            <div class="mt-3">
                <div class="form-inline items-start flex-col xl:flex-row mt-5 pt-5 first:mt-0 first:pt-0">
                    ...
                    <div class="w-full mt-3 xl:mt-0 flex-1">
                        <select id="" name="floor_id" class="tom-select" required>
                            @foreach ($floors as $item)
                            <option value="{{ $item->id }}">{{ $item->loginlocation->location_name }} - {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="mt-3">
                <div class="form-inline items-start flex-col xl:flex-row mt-5 pt-5 first:mt-0 first:pt-0">
                    ...
                    <div class="w-full mt-3 xl:mt-0 flex-1">
                        <input id="" name="name" type="text" class="form-control" placeholder="Room name" required>
